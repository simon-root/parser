...
First of all, you need to use `go get` to fetch the dependencies through git hash. The git hashes are available in [release page](https://github.com/simon-root/parser/v5/releases). Take `v4.0.2` as an example:

```bash
go get -v github.com/simon-root/parser/v5@3a18f1e
```
...
1. Use the [`parser.New()`](https://pkg.go.dev/github.com/simon-root/parser/v5?tab=doc#New) function to instantiate a parser, and
2. Invoke the method [`Parse(sql, charset, collation)`](https://pkg.go.dev/github.com/simon-root/parser/v5?tab=doc#Parser.Parse) on the parser.
...
import (
	...
	_ "github.com/simon-root/parser/v5/test_driver"
)
